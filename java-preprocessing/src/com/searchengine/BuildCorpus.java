...
import com.searchengine.utils.ProgressLogger;
import java.io.*;
...
public class BuildCorpus {
    ...
    static int MAX_DICTIONARY_SIZE = 50000;
    ...
    private static String dicoFilePath = BASE_RESOURCES.resolve("java-processed/top_words.txt").toString();
    private static String wordPageRelationPath = BASE_RESOURCES.resolve("java-processed/word-page-relation.txt").toString();
    ...
    public void buildCorpus() {
        try {
            ...
            //    Path path = BASE_RESOURCES.resolve("raw/frwiki-20230301-pages-articles.xml");
            //    Path path = BASE_RESOURCES.resolve("raw/frwiki10000.xml");

            // very powerful to use BufferedReader and very performant
            FileReader fileReader = new FileReader(sourceFilePath, StandardCharsets.UTF_8);
            ...
            ProgressLogger logger = new ProgressLogger("BUILD CORPUS");

            // very powerful to use BufferedWriter and very performant
            BufferedWriter bufferedWriter = new BufferedWriter(new FileWriter(corpusPath, StandardCharsets.UTF_8));

            XMLOutputFactory factory = XMLOutputFactory.newInstance();
            XMLStreamWriter writer = factory.createXMLStreamWriter(bufferedWriter); // we can use instead new FileOutputStream(BASE_RESOURCES.resolve("java-processed/corpus.xml").toString()) but not very powerful and its slow
    ...
    public void buildDico() {
        try {
            ...
            ProgressLogger logger = new ProgressLogger("BUILD DICTIONARY");
            ...
            BufferedWriter w = new BufferedWriter(new FileWriter(dicoFilePath, StandardCharsets.UTF_8));
            int count = 0;
    ...
    public void buildWordRelation() {
        try {
            ...
            ProgressLogger logger = new ProgressLogger("BUILD WORD-PAGE RELATION");

            FileReader dicoFile = new FileReader(dicoFilePath, StandardCharsets.UTF_8);
            ...
            String line;
            String[] dicoWords = new String[MAX_DICTIONARY_SIZE];
            int i = 0;
            ...
            BufferedWriter w = new BufferedWriter(new FileWriter(wordPageRelationPath, StandardCharsets.UTF_8));
    ...
    public static void pageIdToTitle(String srcFile, String destFile) {
        try {
            ...
            FileReader fileReader = new FileReader(srcFile, StandardCharsets.UTF_8);
            BufferedReader bufferedReader = new BufferedReader(fileReader);
            XMLInputFactory xmlInputFactory = XMLInputFactory.newInstance();
            xmlInputFactory.setProperty("jdk.xml.totalEntitySizeLimit", "0");
            XMLEventReader reader = xmlInputFactory.createXMLEventReader(bufferedReader);// we can use instead new FileInputStream(path.toFile() but not very powerful and its slow

            ProgressLogger logger = new ProgressLogger("BUILD WORD RELATION");
            ...
            BufferedWriter w = new BufferedWriter(new FileWriter(destFile, StandardCharsets.UTF_8));
    ...
    public static int calculatePageNumber(String filePath) {
        ...
        try {
            ...
            // ProgressLogger logger = new ProgressLogger("CALCULATE NUMBER OF PAGES");
