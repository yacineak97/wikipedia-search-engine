...
word_maxs = {}
with open(BASE_DIR / "resources" / "python-processed" / "word-maxs.txt", "r", buffering=8192, encoding='utf-8') as file:
    for line in file:
        lineCleaned = line.strip()
        word = lineCleaned.split(":")[0]
        word = word.lower()
...
page_id_to_title = {}
with open(BASE_DIR / "resources" / "java-processed" / "pageid_title.txt", "r", buffering=8192, encoding='utf-8') as file:
    for line in file:
        lineCleaned = line.strip()
        page_id = int(lineCleaned.split(":")[0])
...
i = 0
word_pages_relation = {}
with open(BASE_DIR / "resources" / "python-processed" / "tf.txt", "r", buffering=1024 * 1024, encoding='utf-8') as file:
    for line in file:
        lineCleaned = line.strip()
        word = lineCleaned.split(":")[0]
        word = word.lower()
...
def index():
    if request.method == 'POST':
        ...
        return render_template('results.html', query=query, results=page_results,
